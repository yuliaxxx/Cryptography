...
# Задает начало алгоритма
def Start():
    try:
        mb.showinfo("Информация", "Сгенирируйте ключ")
...
# Генерирует псевно-случайный ключ
def generate_key():
    try:
...
# Очистка формы
def clearFrame():
    start_butt.configure(state='active')
...
if __name__ == "__main__":
    ...
    position = {'ipadx': 5, 'ipady': 2, 'padx': 10, 'pady': 10, 'sticky': 'nswe'}

    start_butt = tkinter.Button(root)
    start_butt.configure(text='Начать', font='Arial 10', command=Start)
    start_butt.grid(row=1, column=1, columnspan=3, **position)
    ...
    generate_keys.configure(text='Сгенерировать', font='Arial 10', command=generate_key)
    ...
    encrypt_text.configure(text='Зашифровать', font='Arial 10', command=encryptText)
    encrypt_text.configure(state='disabled')
    encrypt_text.grid(row=6, column=1, **position)

    decrypt_text = tkinter.Button(root)
    decrypt_text.configure(text='Расшифровать', font='Arial 10', command=decryptText)
    decrypt_text.configure(state='disabled')
